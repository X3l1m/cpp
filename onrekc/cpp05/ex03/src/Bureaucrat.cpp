...
void	Bureaucrat::executeForm(AForm const & form)
{
	...
	{
		form.execute(*this);
		std::cout << C_BLUE << this->__name << C_GREEN " executed " C_CYAN << form.getName() << C_RESET << std::endl;
	}
	catch(const std::exception& e)
	{
		std::cout << C_BLUE << this->__name << C_RED " couldn't execute " C_CYAN << form.getName() << C_RESET << " form because, " << e.what() << std::endl;
	}
}
